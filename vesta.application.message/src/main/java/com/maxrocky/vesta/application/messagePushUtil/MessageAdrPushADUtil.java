 ...
 */
public class MessageAdrPushADUtil {

    public int messageUserAdrPush(List<MessagePushDTO> messagePushDtos,String userType) throws  Exception{

        String apiKey=AppConfig.ANDROIDAPIKEY;
        String secretKey=AppConfig.ANDROIDSECRETKEY;
        ...
        try {
            ...
            if (messagePushDtos.size()>0){
                for (MessagePushDTO messagePushDTO:messagePushDtos){
                    ...
                    jsonCustormCont.put("params",messagePushDTO.getmPushID());
            ...
            if (requests.size()>0){
                for (PushMsgToSingleDeviceRequest request:requests){
                    ...
                    System.out.println("推送安卓-----------------" + request.getChannelId());
                }
            }
