...
<script>
    $(function(){
        console.log("sqq")
        $("#005500060000").addClass("active");
        $("#005500060000").parent().parent().addClass("in");
        $("#005500060000").parent().parent().parent().parent().addClass("active");
    })
    new WOW().init();
</script>
...
<div class="main-content">
    <vl:leftmenu sysTitle="Vesta Dashboard" menulist="<%=menulist%>" secanViewlist="<%=secanViewlist%>"
                 crunMenu="005500060000" username="${propertystaff.staffName}"/>
    <div class="forms">
        <div class="widget-shadow " data-example-id="basic-forms">
            <%--搜索条件开始--%>
            <div class="form-body form_b">
                <form id="form" class="form-horizontal" action="../propertyManage/getPropertyManageList.html">
                    <input type="hidden" id="menuId" name="menuId" value="005500060000"/>
                    <!-- 城市 -->
                            ...
                            <select id="scopeId" name="scopeId" class="form-control" onchange="queryProjectNameById()">
                                ...
                                <c:forEach items="${city}" var="item" >
                                    <option value="${item.cityId }"
                                            <c:if test="${item.cityId eq propertyManageDTO.scopeId}">selected</c:if>>${item.cityName }</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-8">
                            <select id="projectCode" name="projectCode" class="form-control">
                                <c:forEach items="${projectList}" var="item" >
                                    <option value="${item[0] }"
                                            <c:if test="${item[0] eq propertyManageDTO.projectCode}">selected</c:if>>${item[1] }</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
                            ...
                            <select class="form-control" id="blockCode" name="blockCode">
                                <c:forEach items="${areaMap}" var="item" >
                                    <option value="${item.key }"
                                            <c:if test="${item.key eq propertyManageDTO.blockCode}">selected</c:if>>${item.value }</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-8">
                            <select class="form-control" placeholder="" id="buildingId" name="buildingNum">
                                <c:forEach items="${buildMap}" var="building">
                                    <option value="${building.key}" <c:if test="${building.key eq propertyManageDTO.buildingNum}">selected</c:if>>${building.value}</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-8">
                            <select class="form-control" placeholder="" id="unitId" name="unitCode">
                                <c:forEach items="${unitMap}" var="unit">
                                    <option value="${unit.key}" <c:if test="${unit.key eq propertyManageDTO.unitCode}">selected</c:if>>${unit.value}</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group  col-xs-4">
                        ...
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="roomName" name="roomName" value="${propertyManageDTO.roomName}">
                        </div>
                    </div>
                    <!-- 业主姓名 -->
                    <div class="form-group  col-xs-4">
                        ...
                        <div class="col-sm-8">
                            <input type="text" class="form-control" id="ownerName" name="ownerName" value="${propertyManageDTO.ownerName}">
                        </div>
                    </div>
                    <!-- 办理方式 -->
                    <div class="form-group  col-xs-4">
                        <label for="handleMode" class="col-sm-3 control-label">办理方式</label>
                        <div class="col-sm-8">
                            <select id="handleMode" name="handleMode" class="form-control">
                                <option value="">请选择</option>
                                <option value="AGENCY" <c:if test="${propertyManageDTO.handleMode eq 'AGENCY'}">selected</c:if>>代办</option>
                                <option value="SELF" <c:if test="${propertyManageDTO.handleMode eq 'SELF'}">selected</c:if>>自办</option>
                            </select>
                        </div>
                    </div>
                    <!-- 状态 -->
                    <div class="form-group  col-xs-4">
                        ...
                        <div class="col-sm-8">
                            <select id="handleStatus" name="handleStatus" class="form-control">
                                <option value="">请选择</option>
                                <option value="0" <c:if test="${propertyManageDTO.handleStatus eq '0'}">selected</c:if>>正常状态</option>
                                <option value="1" <c:if test="${propertyManageDTO.handleStatus eq '1'}">selected</c:if>>待提交资料</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-xs-12 search_button">
                        ...
                        <%--<a href="../propertyManage/downLoadExcelTemplate" value="" class="btn btn-primary" style="color:#fff" id="downLoadTemplate">下载模板</a>--%>
                    </div>
                </form>
                <form id="uploadForm">
    ...
    <div class="table-responsive bs-example widget-shadow">
        <table width="100%" class="tableStyle">
            ...
            <c:forEach items="${propertyList}" var="property" varStatus="row">
                <tr>
                    ...
                    <td>
                        <c:if test="${property.handleMode eq 'AGENCY'}">代办</c:if>
                        <c:if test="${property.handleMode eq 'SELF'}">自办</c:if>
                    </td>
        ...
        <m:pager pageIndex="${webPage.pageIndex}"
                 ...
                 submitUrl="${pageContext.request.contextPath }/propertyManage/getPropertyManageList.html?menuId=005500060000&pageIndex={0}&cityId=${homeLetterDTO.cityId}&projectCode=${homeLetterDTO.projectCode}&engineeringProgressTitle=${homeLetterDTO.engineeringProgressTitle}"/>
    </div>
...
<script>
    ...
    function doUpload() {
        ...
        $.ajax({
            url: '../propertyManage/uploadPropertyExcel',
            type: 'POST',
            data: formData,
            async: true,
            cache: false,
            ...
            success: function (data) {
                if(data.error == '0'){
                    ...
                    alert(message);
                    window.location.href = "../propertyManage/getPropertyManageList.html?menuId=005500060000";
    ...
    $("#downLoadTemplate").click(function(){
        ...
        window.open("../propertyManage/downLoadExcelTemplateAndData?projectCode="+$("#projectCode").val()+"&blockCode="+$("#blockCode").val());
    });
    //通过城市获取项目列表
