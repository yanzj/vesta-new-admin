...
<div class="main-content">
      ...
      <div class="form-body" style="overflow:hidden;font-size:13px;line-height:25px;font-family:'微软雅黑';">
        <form class="form-horizontal">
          ...
          <div class="form-group  col-lg-4">
            <h4>设备Mac地址：${propertyDoor.macAddress}</h4>
          </div>
        </form>
      </div>
    </div>
  ...
  <div class="forms bs-example">
    <div class="widget-shadow selectTips" data-example-id="basic-forms">
      <%--搜索条件开始--%>
      <div class="form-body form_b">
        <form id="form" class="form-horizontal" action="../property/toAssignUser.html" method="post" enctype="multipart/form-data">
          ...
          <!-- 房间号 -->
          <div class="form-group  col-lg-4">
            <label for="addressDTO" class="col-sm-3 control-label">房间号</label>
            <div class="col-sm-9">
          ...
          <div class="form-group  col-lg-12 search_button">
            ...
            <button type="button" class="btn btn-primary" id="initialize">初始化该门禁权限</button>
  ...
  <div class="table-responsive bs-example widget-shadow">
        ...
        <c:forEach items="${propertyUserDoorList}" var="propertyUserDoor" varStatus="row">
          <tr>
            <td>
              <div class="checkbox">
                <label>
                  <input type="checkbox" class="userIds" value="${propertyUserDoor.userId}" <c:if test="${propertyUserDoor.doorId eq propertyDoor.id}">disabled="disabled"</c:if>/>
                </label>
              </div>
            </td>
            ...
            <td>
              <c:if test="${empty propertyUserDoor.doorId}">未分配</c:if>
              <c:if test="${propertyUserDoor.doorId eq propertyDoor.id}">已分配</c:if>
            </td>
            <td class="last">
              <c:if test="${propertyUserDoor.doorId eq propertyDoor.id}">
                <a href="javascript:void(0);" onclick="assign('0','${propertyUserDoor.userId}')" class="a3">取消分配</a>
              </c:if>
              <c:if test="${empty propertyUserDoor.doorId}">
                <a href="javascript:void(0);" onclick="assign('1','${propertyUserDoor.userId}')" class="a3">分配</a>
              </c:if>
            </td>
          </tr>
        </c:forEach>
    ...
    <m:pager pageIndex="${webPage.pageIndex}"
             ...
             submitUrl="${pageContext.request.contextPath }/property/toAssignUser.html?menuId=${propertyDoorDTO.menuId}&pageIndex={0}&doorId=${propertyDoor.id}&nameDTO=${propertyDoorDTO.nameDTO}&mobileDTO=${propertyDoorDTO.mobileDTO}&addressDTO=${propertyDoorDTO.addressDTO}"/>
  </div>
</div>
</div>
<!-- main content end-->
  ...
  $(function() {
    ...
    $("#initialize").bind('click',function(){
      if(window.confirm("确认要执行初始化操作？")){
        ...
        $.getJSON("/property/initializeUserDoor/${propertyDoor.id}", function (data) {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.error == 0) {
          alert("操作成功！");
          //location.reload();
          window.location.href = "../property/toAssignUser.html?menuId=${propertyDoorDTO.menuId}&pageIndex=1&doorId=${propertyDoor.id}&nameDTO=${propertyDoorDTO.nameDTO}&mobileDTO=${propertyDoorDTO.mobileDTO}&addressDTO=${propertyDoorDTO.addressDTO}";
