...
<head>
    ...
    <script>
        $(function () {
            console.log("sqq")
            $("#002800020000").addClass("active");
            $("#002800020000").parent().parent().addClass("in");
            $("#002800020000").parent().parent().parent().parent().addClass("active");
        })
        new WOW().init();
    </script>
    <!--//end-animate-->
...
<div class="main-content">
    <vl:leftmenu sysTitle="Vesta Dashboard" menulist="<%=menulist%>" secanViewlist="<%=secanViewlist%>"
                 crunMenu="002800020000" username="${authPropertystaff.staffName}"/>
    ...
    <div class="forms">
        <div class="widget-shadow " data-example-id="basic-forms">
            ...
            <div class="form-body">
                <div class="form-marginL">
                    <form class="form-horizontal" name="search" action="../projectKeyProcesses/keyProcessesCount.html"
                      method="GET">
                    <%-- 项目名称 --%>
                        ...
                        <div class="col-sm-8">
                            <select class="form-control" placeholder="" id="tenderId" name="tenderId">
                                <c:forEach items="${typeMaps.tenders}" var="item">
                                    <option value="${item.key }"
                                            <c:if test="${item.key eq keyProcesses.tenderId}">selected</c:if>>${item.value }</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
        ...
        <table width="100%" style="border-top: 1px dashed #ccc;padding-top: 16px;margin-top: -20px;">
            ...
            <tr class="pull-right"
                ...
                <th style="width: 150px;">合格率：${keyProcessesCountList.qualifiedRate}</th>
                <th style="width: 150px;">不合格率：${keyProcessesCountList.unqualifiedRate}</th>
                <th style="width: 150px;">一次通过率：${keyProcessesCountList.onePassRate}</th>
            </tr>
            </thead>
        </table>
    </div>
    <div class="table-responsive bs-example widget-shadow">
        <form style="margin-bottom: 0">
            ...
            <table width="100%" class="tableStyle" style="table-layout: fixed;">
                ...
                <tbody>
                <c:forEach items="${keyProcessesCount}" var="item" varStatus="status">
                    <tr>
                        ...
                        <c:choose>
                            <c:when test="${item.total > 0}">
                                ...
                                <td height="40px"><b>${item.qualifiedTotal}</b></td>
                                <td height="40px"><b>${item.unqualifiedTotal}</b></td>
                                <td height="40px"><b>${item.abnormalShutdown}</b></td>
                                <td height="40px"><b>${item.onePassTotal}</b></td>
                            </c:when>
                            <c:otherwise>
                                <td height="40px">${item.total}</td>
                                <td height="40px">${item.qualifiedTotal}</td>
                                <td height="40px">${item.unqualifiedTotal}</td>
                                <td height="40px">${item.abnormalShutdown}</td>
                                <td height="40px">${item.onePassTotal}</td>
                            </c:otherwise>
                        </c:choose>
                    </tr>
                </c:forEach>
        ...
        <m:pager pageIndex="${webPage.pageIndex}"
                 ...
                 submitUrl="${pageContext.request.contextPath }/projectKeyProcesses/keyProcessesCount.html?pageIndex={0}&projectId=${keyProcesses.projectId}&tenderId=${keyProcesses.tenderId}&buildingId=${keyProcesses.buildingId}"/>
    </div>
</div>
</div>
</div>
...
<script>
    ...
    function isExcel() {
        var size = ${fn:length(keyProcessesCount)};
        if (size > 0) {
            var href = "../projectKeyProcesses/exportCountExcels?projectId=${keyProcesses.projectId}&tenderId=${keyProcesses.tenderId}&buildingId=${keyProcesses.buildingId}";
