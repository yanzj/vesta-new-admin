        ...
        <div class="widget-shadow " data-example-id="basic-forms">
            ...
            <div class="form-body">
                <form class="form-horizontal" name="searchByName" id="searchByName"
                      action="../authorityClient/outerClientUserManage.html">
                    <%--所属机构--%>
                    <div class="form-group  col-lg-4">
                        ...
                        <div style="text-align: center; margin-top: 8px;">
                            <c:if test="${function.qch40010101 eq 'Y'}">
                                <button type="submit" id="sbm" class="btn btn-primary" for="propertySearch">查询</button>
                            </c:if>
                            <c:if test="${function.qch40010102 eq 'Y'}">
                                <a href="../authorityClient/toEditOuterAgency.html?agencyId=${outerUserDTO.agencyIdO}" class="btn btn-primary" >添加组织机构</a>
                            </c:if>
                            <c:if test="${function.qch40010126 eq 'Y'}">
                ...
                <table class="tableStyle  col-md-12">
                    ...
                    <c:forEach items="${outerUserDTOs}" var="list" varStatus="as">
                        <tr>
                            ...
                            <td>
                                <c:if test="${function.qch40010107 eq 'Y'}">
                                <a href="javascript:void(0);" onclick="toEdit('${list.staffIdO}')" id="toEdit"
                                   class="a3">编辑</a>
                                </c:if>
                                <c:if test="${function.qch40010108 eq 'Y'}">
                                ...
                                <c:if test="${function.qch40010109 eq 'Y'}">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $.ajax({
            url: "../authorityClient/getClientOuterAgency",
            dataType: "json",
...
//        $.fn.zTree.init($("#treeDemo"), setting, zNodes);
    ...
    function onClick(event, treeId, treeNode, clickFlag) {
        window.location.href = "../authorityClient/outerClientUserManage.html?agencyIdO=" + treeNode.id ;
        }
    ...
    function batchDele() {
        ...
        if (checkbox) {
            if (confirm("您确认要批量删除已选用户?")) {
                $.ajax({
                    url:"../authorityClient/batchDeleteClientUser",
                    type:"POST",
                    async:false,
                    data:{
                        staffIds:staffIds
...
//                window.location.href = "../authority/outerUserManage.html?agencyIdO=1442cfe2f277454c860b082c9bdf47bc";
    ...
    function downloadModel() {
        if (confirm("是否下载模板？")) {
            var href = "../authorityClient/qcExportOuterPeopleModel";
        ...
        if(hasUser != "0"){
            if(name.length > 0 ){
                if (confirm("确定要删除组织机构：\""+name+"\"吗?")) {
                    $.ajax({
                        type: "GET",
                        url: "../authorityClient/delClientAgencyById?agencyId="+id,
                        cache: false,
                        async: false,
                        dataType: "json",
                        success: function (json) {
                            ...
                            if (data == 'ok') {
                                alert("删除成功！");
                                window.location.href = "../authorityClient/outerClientUserManage.html";
                            } else {
                                alert(data);
                            }
                        }
    ...
    function toDelete(id,state) {
        if(state == "0"){
            if (confirm("确定要删除该账号吗?")) {
                $.ajax({
                    type: "GET",
                    url: "../authorityClient/toDeleteClientUser?staffIdO=" + id,
                    cache: false,
                    async: false,
                    dataType: "json",
                    success: function (data) {
    ...
    function batchStartStaff() {
        ...
        if (checkbox) {
            if (confirm("您确认要批量启用已选用户?")) {
                $.ajax({
                    url:"../authorityClient/batchHandleClientOuterStaff",
                    type:"POST",
                    async:false,
                    data:{
                        state:'1',
...
//                window.location.href = "../authority/outerUserManage.html?agencyIdO=1442cfe2f277454c860b082c9bdf47bc";
    ...
    function toEdit(staffId) {
        window.location.href = "../authorityClient/toEditOuterUser.html?staffIdO="+staffId ;
    }
...
//                window.location.href = "../authority/outerUserManage.html?agencyIdO=1442cfe2f277454c860b082c9bdf47bc";
        ...
        if (excelPath == null || excelPath == '') {
            ...
            if (fileExtend == '.xls' || fileExtend == '.xlsx') {
                if (confirm("您确认要添加'"+excelPath+"'?")) {
                    document.getElementById("frm").action = "../authorityClient/excelAddQCOuterPeople";
    ...
    function isExcel(){
        var size = $("#size").val();
        if(size>0){
            var href = "../authorityClient/outerUserExportExcel?agencyNameO=${outerUserDTO.agencyNameO}&staffNameO=${outerUserDTO.staffNameO}&sysNameO=${outerUserDTO.sysNameO}&stateO=${outerUserDTO.stateO}&mobileO=${outerUserDTO.mobileO}&emailO=${outerUserDTO.emailO}&agencyIdO=${outerUserDTO.agencyIdO}";
            window.location.href = href;
        }else{
            alert("没有可以导出的数据");
        }
