...
<div>
    ...
    <vl:leftmenu  sysTitle="Vesta Dashboard" menulist="<%=menulist%>" secanViewlist="<%=secanViewlist%>" crunMenu="007300010000" username="${authPropertystaff.staffName}" />
            ...
            <div class="row">
                <div class="col-md-10 role_new_submit2">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                        <tr>
                            <td class="role_table_roleName"><label style="color:red;">*</label> 账号</td>
                            <td class="role_table_fillCont">
                                <input type="text"  class="roleNameText" id="sysNameW" name="sysNameW" value="${userManageDTO.sysNameW}"onblur="checkSYSname()">
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td class="role_table_roleName">是否为临时用户</td>
                            <td >
                                <input type="checkbox" value="1" name="temporaryW" <c:if test="${userManageDTO.temporaryW eq 1}">checked="checked"</c:if>>（选中表示为是）
                            </td>
                        </tr>
                        <tr>
...
<script type="text/javascript">
    $.validator.setDefaults({
        submitHandler: function() {
            ...
            var sysNameW = $("#sysNameW").val();
            if(sysNameW=="" || sysNameW==null) {
                alert("用户名不能为空");
                return;
            }
            ...
            if(reg.test(mobile)){
                $.ajax({
                    url: "../authorityClient/addOrUpdateOuterUser",            //目标网址
                    type: "post",
                    async: "false",
                    dataType: "json",
                    ...
                    success: function (json) {
                        ...
                        if (code != 0) {
                            ...
                            if (data == 'ok') {
                                alert("保存成功");
                                window.location.href = '../authorityClient/outerClientUserManage.html?agencyId='+$("#agencyId").val();
                            } else {
                                alert(data);
                            }
                        }
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $.ajax({
            url: "../authorityClient/getClientOuterAgencyByStaffId",
            dataType: "json",
            data:{
    ...
    function checkSYSname() {
        var sysNameW = $("#sysNameW").val();
        $.ajax({
            url: "../authorityClient/checkOuterUser",
            type: 'post',
            async: false,
            dataType: "json",
            data: {
            ...
            success: function (data) {
                if (data.check != 1) {
                    alert("该账号已存在！");
                    document.getElementById("sysNameW").value=""
//                    $("sysNameW").attr("value","");
                    return;
                }
            }
