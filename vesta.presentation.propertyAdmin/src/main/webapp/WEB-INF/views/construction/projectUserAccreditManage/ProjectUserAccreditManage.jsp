...
<head>
    ...
    <script>
        $(function(){
            console.log("sqq")
            $("#007100060000").addClass("active");
            $("#007100060000").parent().parent().addClass("in");
            $("#007100060000").parent().parent().parent().parent().addClass("active");
        })
        new WOW().init();
    </script>
    <!--//end-animate-->
...
<div class="main-content">
    <vl:leftmenu sysTitle="Vesta Dashboard" menulist="<%=menulist%>" secanViewlist="<%=secanViewlist%>"
                 crunMenu="007100060000" username="${authPropertystaff.staffName}"/>
        ...
        <div class="widget-shadow " data-example-id="basic-forms">
            ...
            <div class="form-body">
                <form class="form-horizontal" name="searchByName" id="searchByName"
                      action="../projectAccredit/initProjectUserAccreditManage.html">
                    <%--人员姓名--%>
                    <div class="form-group  col-lg-4">
                        <label for="staffNameA" class="col-sm-4 control-label">人员</label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" placeholder="" id="staffNameA"
                                   name="staffNameA" value="${accreditManageDTO.staffNameA}">
                        </div>
                    </div>
                    <%--授权项目层级--%>
                    <div class="form-group  col-lg-4">
                        <label for="projectNameA" class="col-sm-4 control-label">项目层级</label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" placeholder="" id="projectNameA"
                                   name="projectNameA" value="${accreditManageDTO.projectNameA}">
                        </div>
                    </div>
                        <%--OA账号--%>
                        <div class="form-group  col-lg-4">
                            <label for="userNameA" class="col-sm-4 control-label">OA账号</label>

                            <div class="col-sm-8">
                                <input type="text" class="form-control" placeholder="" id="userNameA"
                                       name="userNameA" value="${accreditManageDTO.userNameA}">
                            </div>
                        </div>
                    <%--系统账号--%>
                    <div class="form-group  col-lg-4">
                        <label for="sysNameA" class="col-sm-4 control-label">系统账号</label>

                        <div class="col-sm-8">
                            <input type="text" class="form-control" placeholder="" id="sysNameA"
                                   name="sysNameA" value="${accreditManageDTO.sysNameA}">
                        </div>
                    </div>
                    <%--授权角色--%>
                    <div class="form-group  col-lg-4">
                        <label for="roleName" class="col-sm-4 control-label">授权角色</label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" placeholder="" id="roleName"
                                   name="roleNameA" value="${accreditManageDTO.roleNameA}">
                        </div>
                    </div>
                    <div class="form-group  col-lg-4" style="height: 30px;">
                    </div>
                        <div style="text-align: center; margin-top: 8px;">
                            <c:if test="${function.esh40020091 eq 'Y'}">
        ...
        <div class="row">
            ...
            <div class="table-responsive widget-shadow col-md-9" style="float:left;margin-top:0">
                <table class="tableStyle  col-md-12">
                    ...
                    <c:forEach items="${accreditManageDTOS}" var="list" varStatus="as">
                        <tr>
                            <td height="40px" style="text-align: center">
                                <b>${(webPage.pageIndex-1)*20+as.index + 1}</b>
                            </td>
                            <td>${list.staffNameA}</td>
                            <td>${list.sysNameA}</td>
                            <td>${list.userNameA}</td>
                            <td>${list.sourceFromA}</td>
                            <td>${list.modifyOnA}</td>
                            <td>${list.agencyNameA}</td>
                            <td>${list.roleNameA}</td>
                            <td>
                                <c:if test="${list.updateFunction eq 'Y'}">
                                    <a style="cursor: pointer" href="../projectAccredit/userProjectAccreditBatch.html?staffIdA=${list.staffIdA}&staffNameA=${list.staffNameA}" class="a3"><span class="span1">修改</span></a>
                                </c:if>
                                <c:if test="${list.delFunction eq 'Y'}">
                                    <a style="cursor: pointer" href="../projectAccredit/deleteProjectAccredit.html?peojectRoleUserIdA=${list.peojectRoleUserIdA}&agencyIdA=${accreditManageDTO.agencyIdA}&staffNameA=${accreditManageDTO.staffNameA}&userNameA=${accreditManageDTO.userNameA}&sysNameA=${accreditManageDTO.sysNameA}&roleNameA=${accreditManageDTO.roleNameA}&projectNameA=${accreditManageDTO.projectNameA}" class="a3"><span class="span1">删除</span></a>
                                </c:if>
                                </a>
                            </td>
                        </tr>
            ...
            <div class="modal-body"     style="height: auto; min-height: 650px;">
                ...
                <div class="container" style="width: 420px; float: left">
                    ...
                    <div class=" col-md-12 moveBtn" style="margin-top: 3.1%;text-align: center;">
                        <button id="btnR" class="  col-md-2 btn btn-primary" style="float: none;">添加</button>
                    </div>
                        <form class="submitForm" id="form2" name="form2" method="post" action="../projectAccredit/saveUserRoleRelation_2">
                        ...
                        <div  class=" col-md-6 ">
                            <button type="button" onclick="subFrom()" id="sbm2"  for="propertySearch"  class="  col-md-3 btn btn-primary" style= "margin-left: 35%; margin-top: 20px;">提交</button>
                        </div>
                        <div class="  col-md-6 ">
                            <button for="propertySearch" data-dismiss="modal" class="  col-md-3 btn btn-primary" style= "margin-left: 35%; margin-top: 20px;">取消</button>
                        </div>
                    </div>
                    </div>
                </div>
...
<script type="text/javascript">
    $('#myModal1').on('shown.bs.modal', function (e) {
        $(".modal-body").prepend(' <div id="step"></div>');
        var $step = $("#step");
        var $index = $("#index");
        $step.step({
            index: 0,
            time: 500,
            title: ["开始","选择角色", "选择用户", "预授权", "授权完成"]
        });
    ...
    $('.serach_people').bind('click', function () {
        ...
        $.ajax({
            url: '/projectAccredit/getUserByName',
            data: {
    ...
    $('.serach_project').bind('click', function () {
        ...
        $.ajax({
            url: '/projectAccredit/getRoleByNameAndProjectId',
            data: {
...
//        arrData2select(arr2, '#right2');
    ...
    $(document).ready(function(){
        $.ajax({
            url: "../projectAccredit/getOAAgencyMessage",
            dataType: "json",
            success: function (result) {
                $.fn.zTree.init($("#treeDemo1"), setting1, result);
                var cId = "${accreditManageDTO.agencyIdA}";
                var treeO = $.fn.zTree.getZTreeObj("treeDemo1");
        ...
        $.ajax({
            url: "../projectAccredit/getAllAgencyManageById",
            dataType: "json",
            async: false,
            cache: false,
...
//                $.fn.zTree.init($("#treeDemo"), setting,result);
    ...
    function onClick1(event, treeId, treeNode, clickFlag) {
        window.location.href = "../projectAccredit/initProjectUserAccreditManage.html?agencyIdA=" + treeNode.id ;
    }
    function onClick(event, treeId, treeNode, clickFlag) {
        var childrenArr;
        var leftId = '#left'
        //是最后一个  找后台要下一级
        $.getJSON('/projectAccredit/getProjectUserByAgencyId?id=' + treeNode.id , function (res) {
    ...
    function onClick2(event, treeId, treeNode, clickFlag) {
        ...
        var leftId = '#left2'
        //是最后一个  找后台要下一级
        $.getJSON('/projectAccredit/getProjectRoleByAgencyId?id=' + treeNode.id , function (res) {
            childrenArr = res.data;
