...
<div class="main-content">
  ...
  <vl:leftmenu  sysTitle="Vesta Dashboard" menulist="<%=menulist%>" secanViewlist="<%=secanViewlist%>" crunMenu="002900010000" username="${authPropertystaff.staffName}" />
  <div class="forms">
    <div class="widget-shadow " data-example-id="basic-forms">
      <div class="form-body">
        <div class="form-marginL">
          <form class="form-horizontal" action="../ProjectMaterial/projectMaterial.html" method="get">
            <%--集团--%>
          ...
          <div class="form-group  col-lg-4">
            ...
            <div class="col-sm-8">
              <select class="form-control" placeholder="" id="state" name="state">
                ...
                <option value="已退场" <c:if test="${problem.state eq '已退场'}">selected</c:if>>已退场</option>
              </select>
            </div>
          </div>
          <%-- 一级分类 --%>
          ...
          <div class="clearfix"></div>
            <div style="text-align: center; margin-top: 8px;">
              <c:if test="${function.esh40020022 eq 'Y'}">
              ...
              <c:if test="${function.esh40020023 eq 'Y'}">
    ...
    <table width="100%" class="tableStyle" style="table-layout: fixed;">
      ...
      <c:forEach items="${problems}" var="problem" varStatus="row">
        <tr>
          ...
          <td height="40px">${problem.classTwoName}</td>
          <td height="40px">${problem.approachTime}</td>
          <td height="40px">${problem.supplierName}</td>
          ...
          <td height="40px">
            <c:if test="${function.esh40020024 eq 'Y'}">
              <a href="../ProjectMaterial/getMaterialBymaterialId?materialId=${problem.materialId}&projectId=${problem.projectId}"><spring:message code="problem.detail"/></a>
            </c:if>
          </td>
        </tr>
      </c:forEach>
...
<script>
  function isExcel() {
    ...
    if(size>0){
      var href = "../ProjectMaterial/exportExcel?groupId=${problem.groupId}&regionId=${problem.regionId}&cityId=${problem.cityId}&projectId=${problem.projectId}&state=${problem.state}&classifyOne=${problem.classifyOne}&classifyTwo=${problem.classifyTwo}&supplier=${problem.supplier}&firstPartyName=${problem.firstPartyName}&supervisorName=${problem.supervisorName}&assignName=${problem.assignName}&startDate=${problem.startDate}&endDate=${problem.endDate}";
  ...
  $("#classifyOne").change(function(){
    ...
    $.ajax({
      url:"../ProjectMaterial/getSecondLevel",
      type:"get",
      async:"false",
