    ...
    <div class="forms">
        <div class="widget-shadow " data-example-id="basic-forms">
            ...
            <div class="form-body form_b">
                <form:form class="form-horizontal" action="../userFeed/appealManage.html"
                           modelAttribute="feedBackSearch" method="get">
                    <input type="hidden" id="menuId" name="menuId" value="005200010000"/>
                    <%--
                    <div class="form-group  col-lg-4">
                        <label for="queryScope" class="col-sm-4 control-label"><spring:message
                                code="ownerManage.Project"/></label>

                        <div class="col-sm-8">
                            <form:input type="text" class="form-control" placeholder="请输入项目名称"
                                        value="${feedBackSearch.queryScope}" path="queryScope"/>
                        </div>
                    </div>
                    --%>
                    <!-- 城市 -->
                    ...
                    <div class="form-group  col-lg-4">
                        <label for="userType" class="col-sm-4 control-label"><spring:message
                                code="ownerManage.UserType"/></label>
                    ...
                    <div class="form-group  col-lg-4">
                        <label for="status" class="col-sm-4 control-label"><spring:message
                                code="billInfo.processStatus"/></label>
                    ...
                    <div class="form-group  col-lg-4">
                        <label for="memo" class="col-sm-4 control-label"><spring:message
                                code="workOrders.repairWay"/></label>
                    ...
                    <div class="form-group  col-lg-4">
                        ...
                        <div class="col-sm-8">
                            <form:input type="text" class="form-control" placeholder="请输入手机号"
                                        value="${feedBackSearch.phone}" path="phone"/>
                        </div>
                    </div>
                    <div class="form-group  col-lg-4">
                        <label for="content" class="col-sm-4 control-label"><spring:message
                                code="operationLog.content"/></label>

                        <div class="col-sm-8">
                            <form:input type="text" class="form-control" placeholder="请输入内容"
                                        value="${feedBackSearch.content}" path="content"/>
                        </div>
                    </div>
                    <div class="form-group  col-lg-4">
                        <label for="startDate" class="col-sm-4 control-label">申诉时间</label>

                        <div class="input-group date form_date col-sm-8" data-date="" data-date-format="yyyy-mm-dd"
                             data-link-field="dtp_input2">
                            <form:input type="text" class="form-control" placeholder="请输入开始时间" path="startDate"
                                        value="${feedBackSearch.startDate}" readonly="true"/>
                    ...
                    <div class="form-group  col-lg-4">
                        ...
                        <div class="input-group date form_date col-sm-8" data-date="" data-date-format="yyyy-mm-dd"
                             data-link-field="dtp_input2">
                            <form:input type="text" class="form-control" placeholder="请输入结束时间" path="endDate"
                                        value="${feedBackSearch.endDate}" readonly="true"/>
                            <span class="input-group-addon myTime"><span
                                    class="glyphicon glyphicon-remove"></span></span>
                            <span class="input-group-addon myTime"><span
                                    class="glyphicon glyphicon-calendar"></span></span>
    ...
    <div class="table-responsive bs-example widget-shadow">
        ...
        <table width="100%" class="tableStyle table-striped">
            ...
            <tr>
                ...
                <th>房间号</th>
                <th><spring:message code="ownerManage.feedbackTime"/></th>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${feed.state ==1}">
                                <%--<a href="../userFeed/appealUpdate.html?id=${feed.id}"><spring:message code="userFeed.deal" /></a><!--处理-->--%>
                                <a href="javascript:void(0);" onclick="js_handle('${feed.id}')"><spring:message
                                        code="userFeed.deal"/></a>
                            </c:when>
                            <c:otherwise>
                                <a href="javascript:void(0);"
                                   onclick="javascript:alert('处理状态只能在未处理时可编辑！');"><spring:message
                                        code="userFeed.deal"/></a><!--处理-->
                            </c:otherwise>
                        </c:choose>
                        <a href="../userFeed/appealDetail.html?id=${feed.id}"><spring:message
        ...
        <m:pager pageIndex="${webPage.pageIndex}"
                 ...
                 submitUrl="${pageContext.request.contextPath }/userFeed/appealManage.html?menuId=005200010000&pageIndex={0}&scopeId=${feedBackSearch.scopeId}&projectCode=${feedBackSearch.projectCode}&userType=${feedBackSearch.userType}&status=${feedBackSearch.status}&memo=${feedBackSearch.memo}&phone=${feedBackSearch.phone}&content=${feedBackSearch.content}&startDate=${feedBackSearch.startDate}&endDate=${feedBackSearch.endDate}"/>
    </div>
</div>
</div>
</div>
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.error == 1) {
                                $.ajax({
                                    type: "GET",
                                    url: "../userFeed/searchFeedback/" + id + "/",
                                    cache: false,
                                    async: false,
                                    dataType: "json",
                                    success: function (data) {
                                        if (data.data.success) {
                                            window.location.href = "../userFeed/appealUpdate.html?id=" + id;
                                        } else if (data.data.error) {
                                            alert("系统提示：" + data.data.error);
                                            //此处业主申诉后期会追加反写CRM业主操作。
                                            /*
                                            var judge = confirm("系统提示：" + data.data.error + "，确定要操作吗？");
                                            if (judge == true) {
                                                window.location.href = "../userFeed/appealUpdate.html?id=" + id;
                                            } else {
                                                return;
...
//                window.location.href = "../userFeed/appealUpdate.html?id=" + id;
    ...
    function isExcel() {
        var size = $("#size").val();
        if (size > 0) {
            var href = "../userFeed/exportExcel?type=2&pageIndex={0}&scopeId=${feedBackSearch.scopeId}&projectCode=${feedBackSearch.projectCode}&userType=${feedBackSearch.userType}&status=${feedBackSearch.status}&memo=${feedBackSearch.memo}&phone=${feedBackSearch.phone}&content=${feedBackSearch.content}&startDate=${feedBackSearch.startDate}&endDate=${feedBackSearch.endDate}";
