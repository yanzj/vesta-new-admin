...
<head>
    ...
    <script>
        $(function(){
            console.log("sqq")
            $("#005700050000").addClass("active");
            $("#005700050000").parent().parent().addClass("in");
            $("#005700050000").parent().parent().parent().parent().addClass("active");
        })
        new WOW().init();
    </script>
</head>
...
<div class="main-content">
    <vl:leftmenu sysTitle="Vesta Dashboard" menulist="<%=menulist%>" secanViewlist="<%=secanViewlist%>"
                 crunMenu="005700050000" username="${propertystaff.staffName}"/>
    <div class="forms">
        <div class="widget-shadow " data-example-id="basic-forms">
            <%--搜索条件开始--%>
            <div class="form-body form_b">
                <form id="form" class="form-horizontal" action="../visitor/getVisitorPassList.html">
                    <input type="hidden" id="menuId" name="menuId" value="005700050000"/>
                    <!-- 城市 -->
    ...
    <div class="table-responsive bs-example widget-shadow">
        <table width="100%" class="tableStyle">
            ...
            <tbody>
            <c:forEach items="${visitorPassList}" var="visitorPass" varStatus="row">
                <tr>
                    <td><b>${row.index + 1}</b></td>
                    <td>${visitorPass.projectName}</td>
                    <td>${visitorPass.houseAddress}</td>
                    <td>${visitorPass.ownerName}</td>
                    <td>${visitorPass.ownerPhone}</td>
                    <td>${visitorPass.visitorName}</td>
                    <td>${visitorPass.visitorPhone}</td>
                    <td><fmt:formatDate type="time" value="${visitorPass.visitDate}" pattern="yyyy-MM-dd" /></td>
                    <td>${visitorPass.createOn}</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
