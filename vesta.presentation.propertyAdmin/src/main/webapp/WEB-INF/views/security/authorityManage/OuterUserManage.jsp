...
<div class="main-content">
    ...
    <div style="text-align: right; margin-top: 8px; margin-right: 25px;">
        <c:if test="${function.sth40020000 eq 'Y'}">
            <a href="../authority/initUserManage.html" class="btn btn-primary">内部用户管理</a>
        </c:if>
        <a href="../authority/outerUserManage.html" class="btn btn-primary">外部用户管理</a>
        <c:if test="${function.sth40020017 eq 'Y'}">
            <a href="../authority/getEnabledUser.html" class="btn btn-primary">系统用户管理</a>
        </c:if>
    </div>
    <div class="forms">
        <div class="widget-shadow " data-example-id="basic-forms">
            ...
            <div class="form-body">
                <form class="form-horizontal" name="searchByName" id="searchByName"
                      action="../authority/outerUserManage.html">
                    <%--所属机构--%>
                    <div class="form-group  col-lg-4">
                        ...
                        <div style="text-align: center; margin-top: 8px;">
                            <c:if test="${function.sth40020008 eq 'Y'}">
                                <button type="submit" id="sbm" class="btn btn-primary" for="propertySearch">查询</button>
                            </c:if>
                            <c:if test="${function.sth40020009 eq 'Y'}">
                                <a href="../authority/toEditOuterAgency.html?agencyId=${outerUserDTO.agencyIdO}" class="btn btn-primary" >添加组织机构</a>
                            </c:if>
                            <c:if test="${function.sth40020033 eq 'Y'}">
                ...
                <table class="tableStyle  col-md-12">
                    ...
                    <c:forEach items="${outerUserDTOs}" var="list" varStatus="as">
                        <tr>
                            ...
                            <td>
                                <c:if test="${function.sth40020014 eq 'Y'}">
                                <a href="javascript:void(0);" onclick="toEdit('${list.staffIdO}')" id="toEdit"
                                   class="a3">编辑</a>
                                </c:if>
                                <c:if test="${function.sth40020015 eq 'Y'}">
                                ...
                                <c:if test="${function.sth40020016 eq 'Y'}">
...
//        $.fn.zTree.init($("#treeDemo"), setting, zNodes);
    ...
    function onClick(event, treeId, treeNode, clickFlag) {
        window.location.href = "../authority/outerUserManage.html?agencyIdO=" + treeNode.id ;
        }
...
//                window.location.href = "../authority/outerUserManage.html?agencyIdO=1442cfe2f277454c860b082c9bdf47bc";
            } else {
                return;
            }
        } else {
    ...
    function downloadModel() {
        if (confirm("是否下载模板？")) {
            var href = "../authority/exportOuterPeopleModel";
        ...
        if(hasUser != "0"){
            if(name.length > 0 ){
                if (confirm("确定要删除组织机构：\""+name+"\"吗?")) {
                    $.ajax({
                        type: "GET",
                        url: "../authority/delAgencyById?agencyId="+id,
                        cache: false,
                        async: false,
                        dataType: "json",
                        success: function (json) {
                            ...
                            if (data == 'ok') {
                                alert("删除成功！");
                                window.location.href = "../authority/outerUserManage.html";
                            } else {
                                alert(data);
                            }
                        }
    ...
    function toDelete(id,state) {
        if(state == "0"){
            if (confirm("确定要删除该账号吗?")) {
                $.ajax({
                    type: "GET",
                    url: "../authority/toDeleteUser?staffIdO=" + id,
                    cache: false,
                    async: false,
                    dataType: "json",
                    success: function (data) {
    ...
    function batchStartStaff() {
        ...
        if (checkbox) {
            if (confirm("您确认要批量启用已选用户?")) {
                $.ajax({
                    url:"../authority/batchHandleStaff",
                    type:"POST",
                    async:false,
                    data:{
                        state:'1',
...
//                window.location.href = "../authority/outerUserManage.html?agencyIdO=1442cfe2f277454c860b082c9bdf47bc";
    ...
    function toEdit(staffId) {
        window.location.href = "../authority/toEditOuterUser.html?staffIdO="+staffId ;
    }
...
//                window.location.href = "../authority/outerUserManage.html?agencyIdO=1442cfe2f277454c860b082c9bdf47bc";
        ...
        if (excelPath == null || excelPath == '') {
            ...
            if (fileExtend == '.xls' || fileExtend == '.xlsx') {
                if (confirm("您确认要添加'"+excelPath+"'?")) {
                    document.getElementById("frm").action = "../authority/excelAddOuterPeople";
    ...
    function isExcel(){
        var size = $("#size").val();
        if(size>0){
            var href = "../authority/outerUserExportExcel?agencyNameO=${outerUserDTO.agencyNameO}&staffNameO=${outerUserDTO.staffNameO}&sysNameO=${outerUserDTO.sysNameO}&stateO=${outerUserDTO.stateO}&mobileO=${outerUserDTO.mobileO}&emailO=${outerUserDTO.emailO}&agencyIdO=${outerUserDTO.agencyIdO}";
            window.location.href = href;
        }else{
            alert("没有可以导出的数据");
        }
