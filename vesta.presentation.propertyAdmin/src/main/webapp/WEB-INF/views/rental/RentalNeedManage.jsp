...
<div class="forms">
    <div class="widget-shadow " data-example-id="basic-forms">
        <%--搜索条件开始--%>
        <div class="form-body">
            <form class="form-horizontal" action="../rental/rentalNeed.html">
...
<th><spring:message code="rental.formatName" /></th>
<th><spring:message code="rental.uname" /></th>
<th><spring:message code="rental.sex" /></th>
<th><spring:message code="rental.mobilephone" /></th>
<%--<th><spring:message code="rental.lookTime" /></th>--%>
<th><spring:message code="rental.moneyfor" /></th>
<th><spring:message code="rental.wanthouseType" /></th>
<th><spring:message code="rental.desciption" /></th>
<th><spring:message code="rental.timeview" /></th>
    ...
    <c:forEach items="${list}" var="rentalList" varStatus="row">
        <tr>
            ...
            <td>
                <c:if test="${rentalList.sex==1}">男</c:if>
                <c:if test="${rentalList.sex==2}">女</c:if>
            </td>
            <td>${rentalList.mobile}</td>
            <%--<td>${rentalList.lookTime}</td>--%>
            <td>${rentalList.minMoney}&nbsp;-&nbsp;${rentalList.maxMoney}</td>
            <td>${rentalList.houseType}</td>
            <td>
                <c:choose>
                    <c:when test="${fn:length(rentalList.description) > 15}">
                        <c:out value="${fn:substring(rentalList.description, 0, 15)}......" />
                    </c:when>
                    <c:otherwise>
                        <c:out value="${rentalList.description}" />
                    </c:otherwise>
                </c:choose>
            </td>
            <td>${rentalList.crtTime}</td>
            <td>
                <a style="cursor: pointer" onclick="javascript:if(confirm('确定删除吗！'))location.href='../rental/deleteRental.html?rentalId=${rentalList.rentalId}&type=2';else return"><spring:message code="common_delete" /></a>
            </td>
        </tr>
    </c:forEach>
    </tbody>
    ...
    <m:pager pageIndex="${webPage.pageIndex}"
             ...
             submitUrl="${pageContext.request.contextPath }/rental/rentalNeed.html?pageIndex={0}" />
    </div>
