...
<div class="main-content">
    ...
    <div class="forms">
        <div class="widget-shadow " data-example-id="basic-forms">
            ...
            <div class="form-body form_b">
                <form id="form" class="form-horizontal" action="../story/getStoryInfoList.html">
                            ...
                            <select id="scopeId" name="cityId" class="form-control" onchange="queryProjectNameById()">
                                ...
                                <c:forEach items="${city}" var="item" >
                                    <option value="${item.cityId }"
                                            <c:if test="${item.cityId eq storyDTO.cityId}">selected</c:if>>${item.cityName }</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-8">
                            <select id="projectCode" name="projectNum" class="form-control">
                                <c:forEach items="${projectList}" var="item" >
                                    <option value="${item[0] }"
                                            <c:if test="${item[0] eq storyDTO.projectNum}">selected</c:if>>${item[1] }</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
                    <%-- 故事荟标题 --%>
                    <div class="form-group  col-xs-4">
                        <label for="title" class="col-sm-4 control-label">故事荟</label>
                        <div class="col-sm-8">
                            <input type="text" class="form-control" placeholder="" id="title" name="title" value="${storyDTO.title}">
                        </div>
                    </div>
                    <%-- 发布人 --%>
                    <div class="form-group  col-xs-4">
                        ...
                        <div class="col-sm-8">
                            <input type="text" class="form-control" placeholder="" id="releasePerson" name="releasePerson" value="${storyDTO.releasePerson}">
                        </div>
                    </div>
                    <!-- 发布开始日期 -->
                    <div class="form-group  col-xs-4">
                        ...
                        <div class="input-group date form_date col-sm-8" data-date="" data-date-format="yyyy-mm-dd"
                            ...
                            <input type="text" class="form-control" placeholder="" id="releaseStaDate"
                                   value="${storyDTO.releaseStaDate}" name="releaseStaDate" readonly>
                    ...
                    <div class="form-group  col-xs-4">
                        ...
                        <div class="input-group date form_date col-sm-8" data-date="" data-date-format="yyyy-mm-dd"
                            ...
                            <input type="text" class="form-control" placeholder="" id="releaseEndDate"
                                   value="${storyDTO.releaseEndDate}" name="releaseEndDate" readonly>
                            <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                            <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                        </div>
                    </div>
                    ...
                    <div class="form-group  col-xs-4">
                        ...
                        <div class="col-sm-8">
                            <select id="releaseStatus" name="releaseStatus" class="form-control">
                                ...
                                <option value="0" <c:if test="${'0' eq storyDTO.releaseStatus}">selected</c:if>>未发布</option>
                                <option value="1" <c:if test="${'1' eq storyDTO.releaseStatus}">selected</c:if>>已发布</option>
                            </select>
                        </div>
                    </div>
    ...
    <div class="table-responsive bs-example widget-shadow">
        <table width="100%" class="tableStyle">
            ...
            <tbody>
            <c:forEach items="${storyInfoList}" var="storyInfo" varStatus="row">
                <tr>
                    <td><b>${row.index + 1}</b></td>
                    <td>${storyInfo.title}</td>
                    <td>${storyInfo.releaseProjectScopes}</td>
                    <td>
                        <c:if test="${'0' eq storyInfo.releaseStatus}">未发布</c:if>
                        <c:if test="${'1' eq storyInfo.releaseStatus}">已发布</c:if>
                    </td>
                    <td>${storyInfo.releasePerson}</td>
                    <td>${storyInfo.releaseDate}</td>
                    <td class="last">
                        <a href="javascript:void(0);" onclick="toEdit('${storyInfo.id}')" id="toEdit" class="a3">编辑</a>
                        <a href="javascript:void(0);" onclick="toDelete('${storyInfo.id}')" id="toDelete" class="a3">删除</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
        ...
        <m:pager pageIndex="${webPage.pageIndex}"
                 ...
                 submitUrl="${pageContext.request.contextPath }/story/getStoryInfoList.html?menuId=006000030000&pageIndex={0}&cityId=${storyDTO.cityId}&projectNum=${storyDTO.projectNum}&title=${storyDTO.title}&releasePerson=${storyDTO.releasePerson}&releaseStaDate=${storyDTO.releaseStaDate}&releaseEndDate=${storyDTO.releaseEndDate}&releaseStatus=${storyDTO.releaseStatus}"/>
    </div>
    ...
    function toEdit(id){
        $.ajax({
            ...
            success: function (data) {
                if (data.error == 1) {
                    window.location.href = "../story/toEditStoryInfo.html?menuId="+$("#menuId").val()+"&id="+id;
