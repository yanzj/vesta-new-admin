  ...
  <div class="forms">
    <div class="widget-shadow " data-example-id="basic-forms">
      ...
      <div class="form-body form_b">
        <form class="form-horizontal" action="../community/ActivityProjectApply.html">
          ...
          <div class="form-group  col-lg-4">
            <label for="userType" class="col-sm-3 control-label"><spring:message
                    code="activityManage.applyUserType"/>：</label>

            <div class="col-sm-9">
              <select class="form-control" id="userType" name="userType">
                <option value=""> --请选择身份--</option>
                <option value="2" <c:if test="${activityApplyAdminDto.userType eq '2'}">selected</c:if>>
                  普通用户
                </option>
                <option value="3" <c:if test="${activityApplyAdminDto.userType eq '3'}">selected</c:if>>
                  业主
                </option>
              </select>
            </div>
  ...
  <div class="table-responsive bs-example widget-shadow">
    <table width="100%" class="tableStyle">
      ...
      <tr>
        ...
        <%--<th>报名信息</th>--%>
        <c:if test="${activityInfo.applyInfo2 != null}">
          <c:forEach items="${activityInfo.applyInfo2}" var="applyInfo">
            <th>${applyInfo.name}</th>
          </c:forEach>
        </c:if>
        <th>报名人数</th>
      </tr>
      ...
      <c:forEach items="${applyList}" var="apply" varStatus="vs">
        <c:if test="${!empty apply.applyInfoJson}">
          <c:forEach items="${apply.applyInfoJson}" var="applyInfoList">
            <tr>
              <td><b>${vs.count}</b></td>
              <td>
              ...
              <td>
                <c:if test="${fn:contains(apply.makeDate, '00:00')}">
                  <fmt:formatDate type="time" value="${apply.makeDate}" pattern="yyyy-MM-dd" />
                </c:if>
                <c:if test="${!fn:contains(apply.makeDate, '00:00')}">
                  <fmt:formatDate type="time" value="${apply.makeDate}" pattern="yyyy-MM-dd hh:mm:ss" />
                </c:if>
              </td>
              <td>${apply.applyTimeRange}</td>
              <td>${apply.title}</td>
              <c:if test="${applyInfoList != null}">
                <c:forEach items="${applyInfoList}" var="info">
                  <td>${info.value}</td>
                </c:forEach>
              </c:if>
              <td>1</td>
    ...
    <m:pager pageIndex="${webPage.pageIndex}"
             ...
             submitUrl="${pageContext.request.contextPath }/community/ActivityProjectApply.html?menuId=005400040000&pageIndex={0}&activityId=${activityId}&scopeId=${activityApplyAdminDto.scopeId}&projectCode=${activityApplyAdminDto.projectCode}&userName=${activityApplyAdminDto.userName}&userType=${activityApplyAdminDto.userType}"/>
  </div>
</div>
</div>
</div>
...
<script type="text/javascript">
  ...
  function isExcel() {
    var size = $("#size").val();
    if (size > 0) {
      var href = "../community/activityProjectApplyExcel?menuId=005400040000" +
              "&activityId=${activityId}&scopeId=${activityApplyAdminDto.scopeId}&projectCode=${activityApplyAdminDto.projectCode}" +
              "&userName=${activityApplyAdminDto.userName}&userType=${activityApplyAdminDto.userType}";
