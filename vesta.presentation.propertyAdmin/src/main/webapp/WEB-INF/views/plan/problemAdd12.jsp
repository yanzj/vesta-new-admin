...
<head>
    ...
    <script>
        $(function () {
            ...
            $("#001100010000").parent().parent().parent().parent().addClass("active");
        })
        new WOW().init();
    </script>
    <!--//end-animate-->
        ...
        <div class="widget-shadow " data-example-id="basic-forms">
            ...
            <div class="form-body">
                ...
                <div class="form-body" style="min-height:880px">
                    <form class="form-horizontal" id="frm" action="../problem/addProblemManagement" method="post"
                                ...
                                <select class="form-control" placeholder="" id="projectNum" name="projectNum">
                                    <c:forEach items="${typeMaps.projects}" var="item">
                                        <option value="${item.key }"
                                                <c:if test="${item.key eq problem.projectNum}">selected</c:if>>${item.value }</option>
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
                        ...
                        <%--所属计划--%>
                        <div class="form-group col-lg-12">
                            <label class="col-sm-2 control-label" for="planNum"><spring:message
                                    code="problem.plan"/></label>

                            <div class="col-sm-4">
                                <select class="form-control" placeholder="" id="planNum" name="planNum">
                                    <c:forEach items="${typeMaps.planNum}" var="item">
                                        <option value="${item.key }"
                            ...
                            <div class="col-sm-4">
                                <select class="form-control" placeholder="" id="roomNum" name="roomNum">
                                    <c:forEach items="${typeMaps.rooms}" var="item">
                                        <option value="${item.key }"
                                                <c:if test="${item.key eq problem.roomNum}">selected</c:if>>${item.value }</option>
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
                        ...
                        <%--整改时限--%>
                        <div class="form-group  col-lg-12">
                            <label for="limitDate" class="col-sm-2 control-label"><spring:message
                                    code="problem.limit"/></label>
                            <div class="input-group date form_date col-sm-4" data-date="" data-date-format="yyyy-mm-dd"
                                 data-link-field="dtp_input2">
                                <input type="text" class="form-control" placeholder="请选择整改时限" id="limitDate"
                                       name="limitDate" value="${problem.limitDate}" readonly="true"/>
                            ...
                            <div class="col-sm-4">
                                <select class="form-control" placeholder="" id="classifyThree" name="classifyThree">
                                    <c:forEach items="${typeMaps.thirdLevels}" var="item">
                                        <option value="${item.key }"
                                                <c:if test="${item.key eq problem.classifyThree}">selected</c:if>>${item.value }</option>
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-4">
                                <select class="form-control" placeholder="" id="supplier" name="supplier">
                                    <c:forEach items="${typeMaps.supplierMap}" var="item">
                                        <option value="${item.key }"
                                                <c:if test="${item.key eq problem.supplier}">selected</c:if>>${item.value }</option>
                                    </c:forEach>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group col-lg-12">
                            ...
                            <div class="col-sm-4">
                                <textarea class="form-control" id="problemDescription"
                                          name="problemDescription">${problem.problemDescription}</textarea>
                            </div>
                        </div>
                        <div class="form-group col-lg-12">
                            <label class="col-sm-2 control-label" for="problemDescription"><spring:message
                        ...
                        <%--问题图片--%>
                        <div class="form-group col-lg-12">
                            <label class="col-sm-2 control-label" for="imgFile"><spring:message
                                    code="problem.image"/></label>

                            <div class="col-sm-4">
                                <div id="addfileId">
                                    <input type="file" id="f1" name="imgFile" class="form-control"
                                           onchange="check(this)">
                                </div>
                                <div id="demo_imgFile" style="padding-top: 15px;"></div>
                            </div>

                        </div>

                        <input type="hidden" id="problemType" name="problemType">
                        <input type="hidden" id="planType" name="planType" value="12">
                        <input type="hidden" id="rectifyState" name="rectifyState">
                        <input type="hidden" id="roomLocation" name="roomLocation">
                        <input type="hidden" id="xCoordinates" name="xCoordinates">
                        <input type="hidden" id="yCoordinates" name="yCoordinates">

                        <div class='clearfix'></div>
                        <div class="text-center form-group  col-lg-10" style="padding:0;">
                            <c:if test="${function.qch40010022 eq 'Y'}">
                                <button type="button" class="btn btn-primary" onclick="checkSubmit('草稿')">
                                    <spring:message code="problem.save"/></button>
                            </c:if>
                            <c:if test="${function.qch40010023 eq 'Y'}">
                                <button type="button" class="btn btn-primary" onclick="checkSubmit('待接单')">
                                    <spring:message code="problem.submit"/></button>
                            </c:if>
                        </div>
                </div>
...
//          alert(data.data.length);
    ...
    function checkSubmit(rectifyState) {
        ...
        //        新添加的数据校验
        if ($("#classifyOne").val() == "0000") {
            alert("请选择一级分类");
            return false;
        }
        if ($("#classifyTwo").val() == "0000") {
            alert("请选择二级分类");
            return false;
        }
        if ($("#classifyThree").val() == "0000") {
            alert("请选择三级分类");
            return false;
        }
        if ($("#locationName").val() == "") {
        ...
        if ($("#problemDescription").val().length == 0 || $("#problemDescription").val() == "") {
            alert("请输入问题描述");
            return false;
        }
        //        新添加的数据校验
    ...
    function readFile() {
        ...
        for (var i = 0; i < this.files.length; i++) {
            ...
            reader.onload = function (e) {
                result.innerHTML += '<img id="i' + beforAdd + '" onclick="delImg(' + beforAdd + ')" src="' + this.result + '" style="width:100px" alt=""/>';
            }
    ...
    $("#projectNum").change(function(){
        ...
        $.ajax({
            url:"../problem/getPrintPlanListNew",
            type:"get",
            async:"false",
            data:{ "projectNum":projectNum,"planType":"12"},
            dataType:"json",
            error:function(){
                alert("网络异常，可能服务器忙，请刷新重试");
            },
