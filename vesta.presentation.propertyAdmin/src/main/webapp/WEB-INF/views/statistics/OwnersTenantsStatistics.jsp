...
<div class="main-content">
  ...
  <vl:leftmenu  sysTitle="Vesta Dashboard" menulist="<%=menulist%>" secanViewlist="<%=secanViewlist%>" crunMenu="005200050000" username="${propertystaff.staffName}" />
  ...
  <div class="forms">
    <div class="widget-shadow " data-example-id="basic-forms">
      <%--搜索条件开始--%>
      <div class="form-body">
        <form class="form-horizontal" action="../statistics/OwnersTenantsStatistics.html">
            ...
            <div class="col-sm-8">
              <select id="propertyProject" name="propertyProject"  class="form-control">
                <c:forEach items="${propertyCompanyMap.propertyProjectMap}" var="item">
                  <option value="${item.key }" <c:if test="${item.key eq click.propertyProject}">selected="selected"</c:if>>${item.value }</option>
                </c:forEach>
              </select>
            </div>
          </div>
  ...
  <div class="table-responsive bs-example widget-shadow">
    <table width="100%" class="tableStyle">
      ...
      <tr>
        ...
        <%--<th><spring:message code="statistics.roomNumber" /></th>--%>
        <th><spring:message code="statistics.registrationNumber" /></th>
        <th><spring:message code="statistics.relativesNumbert" /></th>
        <th><spring:message code="statistics.tenantsNumbert" /></th>
      </tr>
      </thead>
      <tbody>
      <c:forEach items="${ownersLease}" var="owners" varStatus="row">
        <tr>
          <td><b>${row.index + 1}</b></td>
          <td>${owners.projectName}</td>
          <td>${owners.registrationNumber}</td>
          <td>${owners.relativesNumbert}</td>
          <td>${owners.tenantsNumbert}</td>
        </tr>
      </c:forEach>
      </tbody>
    </table>
    <%--<m:pager pageIndex="${webPage.pageIndex}"
             ...
             submitUrl="${pageContext.request.contextPath }/property/OwnersTenantsStatistics.html?pageIndex={0}&propertyArea=${propertyCompanySearch.propertyArea}&companyName=${propertyCompanySearch.companyName}&propertyProject=${propertyCompanySearch.propertyProject}&projectAdmin${propertyCompanySearch.projectAdmin}"/>
--%>  </div>
