...
<div class="main-content">
  ...
  <vl:leftmenu  sysTitle="Vesta Dashboard" menulist="<%=menulist%>" secanViewlist="<%=secanViewlist%>" crunMenu="000800070000" username="${propertystaff.staffName}" />
  ...
  <div class="forms">
    ...
    <div class="widget-shadow " data-example-id="basic-forms">
      <%--搜索条件开始--%>
      <div class="form-body">
        <form  action="../click/ClickTimesPageManage.html" method="post">
          <div class="form-group  col-lg-3">
            ...
            <div class="col-sm-9">
              <select id="propertyArea" name="propertyArea"  class="form-control">
                <c:forEach items="${ClickTimesMap.propertyAreaMap}" var="item">
          ...
          <div class="form-group  col-lg-4">
            ...
            <div class="col-sm-8">
              <select id="companyName" name="companyName"  class="form-control">
                <c:forEach items="${ClickTimesMap.companyNameMap}" var="item">
                  <option value="${item.key }">${item.value }</option>
                </c:forEach>
              </select>
            </div>
          ...
          <div class="form-group  col-lg-4">
            ...
            <div class="col-sm-8">
              <select id="propertyProject" name="propertyProject"  class="form-control">
                <c:forEach items="${ClickTimesMap.propertyProjectMap}" var="item">
                  <option value="${item.key }">${item.value }</option>
                </c:forEach>
              </select>
            </div>
  ...
  <div class="table-responsive bs-example widget-shadow">
    <table width="100%" class="tableStyle">
      ...
      <tr>
        ...
        <th><spring:message code="clickTimes.statisticsTotal" /></th>
        <th><spring:message code="clickTimes.statisticsProperty" /></th>
        <th><spring:message code="clickTimes.statisticsConsultation" /></th>
        <th><spring:message code="clickTimes.statisticsPraise" /></th>
        <th><spring:message code="clickTimes.statisticsComplaint" /></th>
        <th><spring:message code="clickTimes.statisticsRepair" /></th>
        <th><spring:message code="clickTimes.statisticsEvaluate" /></th>
      </tr>
      </thead>
      <tbody>
      <c:forEach items="${ClickTimesList}" var="click" varStatus="row">
        <tr>
          <td><b>${row.index + 1}</b></td>
            <td>${click.project}</td>
            <td>${click.total}</td>
            <td>${click.property}</td>
            <td>${click.consultation}</td>
            <td>${click.praise}</td>
            <td>${click.complaint}</td>
          <td>${click.repair}</td>
          <td>${click.evaluate}</td>
        </tr>
      </c:forEach>
      </tbody>
    </table>
    <m:pager pageIndex="${webPage.pageIndex}"
             ...
             submitUrl="${pageContext.request.contextPath }/click/ClickTimesPage.html.html?pageIndex={0}&propertyProject=${clickTimesSeachDTO.propertyProject}&companyName=${clickTimesSeachDTO.companyName}&propertyArea=${clickTimesSeachDTO.propertyArea}"/>
  </div>
