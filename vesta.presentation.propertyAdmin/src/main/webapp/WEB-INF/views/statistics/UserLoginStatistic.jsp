...
<div class="main-content">
  ...
  <vl:leftmenu  sysTitle="Vesta Dashboard" menulist="<%=menulist%>" secanViewlist="<%=secanViewlist%>" crunMenu="000800050000" username="${propertystaff.staffName}" />
  ...
  <div class="forms">
    <div class="widget-shadow " data-example-id="basic-forms">
      <%--搜索条件开始--%>
      <div class="form-body">
        <form class="form-horizontal" action="../statistics/UserLoginStatistic.html">
  ...
  <div class="table-responsive bs-example widget-shadow">
    <table width="100%" class="tableStyle">
      ...
      <tr>
        ...
        <th><spring:message code="statistics.registeredNumber" /></th>
        <th><spring:message code="statistics.landingNumber" /></th>
      </tr>
      </thead>
      <tbody>
      <c:forEach items="${userLogin}" var="user" varStatus="row">
        <tr>
          <td><b>${row.index + 1}</b></td>
          <td>${user.projectName}</td>
          <td>${user.registeredNumber}</td>
          <td>${user.landingNumber}</td>
        </tr>
      </c:forEach>
      </tbody>
    </table>
   <%-- <m:pager pageIndex="${webPage.pageIndex}"
             ...
             submitUrl="${pageContext.request.contextPath }/property/UserLoginStatistic.html?pageIndex={0}&propertyArea=${propertyCompanySearch.propertyArea}&companyName=${propertyCompanySearch.companyName}&propertyProject=${propertyCompanySearch.propertyProject}&projectAdmin${propertyCompanySearch.projectAdmin}"/>
 --%>
  </div>
