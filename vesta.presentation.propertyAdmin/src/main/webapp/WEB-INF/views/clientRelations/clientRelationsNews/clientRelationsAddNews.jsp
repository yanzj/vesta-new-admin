    ...
    <div class="forms">
        <div class="widget-shadow " data-example-id="basic-forms">
            <div class="form-body" style="overflow:hidden;font-size:13px;line-height:25px;font-family:'微软雅黑';">
                <form class="form-horizontal" id="formAdd" action="../qcNews/saveOrUpdateNews"
                      method="post"
                      enctype="multipart/form-data">
                    <input type="hidden" id="newsId" name="newsId" value="${newsDTO.newsId}">
                    <input type="hidden" id="newsSource" name="newsSource" value="">
                    <%--项目--%>
                    <div id="projectIds" class="form-group  col-lg-7">
                        <label for="projectId" class="col-sm-3 control-label">新闻所属区域:</label>
                        <div class="col-sm-5">
                            <select class="form-control" placeholder="" id="projectId" name="projectId">
                                <c:forEach items="${projects}" var="project">
                                    <option value="${project.key}"
                                            <c:if test="${project.key eq newsDTO.projectId}">selected</c:if>>${project.value}</option>
                                </c:forEach>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-5" style="margin-top: 7px">
                            <label>
                                <input type="radio" name="newsType"
                                       <c:if test="${newsDTO.newsType eq '1'}">checked</c:if>
                                       value="1">图文</label>
                            <label style="margin-left:20px;">
                                <input type="radio" name="newsType"
                                       <c:if test="${newsDTO.newsType eq '2'}">checked</c:if>
                                       value="2">视频</label>
                            <label style="margin-left:20px;">
                                <input type="radio" name="newsType"
                                       <c:if test="${newsDTO.newsType eq '3'}">checked</c:if>
                                       value="3">链接</label>
                        </div>
                    </div>
                    <%-- 新闻标题 --%>
                    <div class="form-group col-lg-7">
                        ...
                        <div class="col-sm-5">
                            <input type="text" class="form-control" placeholder="" id="newsTitle"
                                   name="newsTitle" value="${newsDTO.newsTitle}">
                        </div>
                    </div>
                    <%-- 新闻展示图 --%>
                    <div class="form-group col-lg-7">
                        <label class="col-sm-3 control-label">新闻封面图:</label>
                        <div class="col-sm-5">
                    ...
                    <div id="newsImg" class="form-group col-lg-7" style="display: none">
                        ...
                        <div class="col-sm-5">
                            <img src="${newsDTO.newsImgUrl}" alt="原新闻展示图" class="img-thumbnail"
                                 style="width: auto">
                        </div>
                    </div>
                    <%-- 视频链接 --%>
                    ...
                    <%-- 发布时间 --%>
                    <div id="isRelease" class="form-group col-lg-7">
                        <label class="col-sm-3 control-label">发布时间:</label>
                        <div class="col-sm-5" style="margin-top: 7px">
                            <label>
                                <input type="radio" name="isRelease"
                                       <c:if test="${newsDTO.isRelease eq '1'}">checked</c:if>
                                       value="1">立即发布</label>
                            <label style="margin-left:20px;">
                                <input type="radio" name="isRelease"
                                       <c:if test="${newsDTO.isRelease eq '0'}">checked</c:if>
                                       value="0">定时发布</label>
                        </div>
                    </div>
                    <%-- 发布日期 --%>
                    <div id="releaseTime" class="form-group col-lg-7">
                        <label class="col-sm-3 control-label" for="releaseDate">发布日期:</label>
                        <div class="col-sm-5">
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        if ($("input[name='newsType']:checked").val() == "2") {
            $("#url").show();
            $("#H5Url").hide();
        } else if ($("input[name='newsType']:checked").val() == "3") {
            $("#url").hide();
            $("#content").hide();
            $("#H5Url").show();
        } else {
        ...
        if ($("input[name='isRelease']:checked").val() == "0") {
            $("#releaseTime").show();
        } else {
            $("#isRelease").hide();
            $("#releaseTime").hide();
        }
        if ($("input[name='isRelease']:checked").val() == null) {
            $("#isRelease").show();
            $("#releaseTime").hide();
        }

        if ($("#projectId option").length == 1) {
            $("#projectIds").hide();
        }
        $("#newsSource").val($("#projectId").find("option:selected").text());
    });
...
<script type="text/javascript">
    ...
    function toSave() {
        ...
        if ($("#projectId").val().replace(/(^\s*)|(\s*$)/g, "").length == 0) {
            alert("请选择新闻所属区域！");
            return;
        }
        ...
        if ($("#newsImgUpload").val().replace(/(^\s*)|(\s*$)/g, "").length == 0) {
            alert("请选择新闻封面图！");
            return;
        }
        if ($("input[name='newsType']:checked").val() == "3") {
            if($("#outUrl").val().replace(/(^\s*)|(\s*$)/g, "").length == 0){
                alert("请输入链接！");
                return;
            }
        }else {
            if ($("input[name='newsType']:checked").val() == "2" && $("#videoUrl").val().replace(/(^\s*)|(\s*$)/g, "").length == 0) {
                alert("请输入视频链接！");
                return;
            }
            if ($("#newsContent").val().replace(/(^\s*)|(\s*$)/g, "").length == 0) {
                alert("请输入新闻内容！");
                return;
            }
        }
        ...
        if ($("input[name='isRelease']:checked").val() == "0" && $("#releaseDate").val().replace(/(^\s*)|(\s*$)/g, "").length == 0) {
            alert("请选择定时发布时间！");
            return;
        }
        $("#formAdd").submit();
...
<script>
    ...
    //发布时间点击事件
    $("input[name='isRelease']").click(function () {
        if (this.defaultValue == '1') {
            $("#releaseTime").hide();
        }
    ...
    //下拉框点击事件
    $("#projectIds select").bind('change', function () {
        $("#newsSource").val($("#projectId").find("option:selected").text());
    });
</script>
<!-- main content end-->
