package com.maxrocky.vesta.application.projectAccredit.inf;
...
/**
 * Created by yuanyn on 2018/03/26.
 */
public interface ProjectUserAccreditService {

    //获取项目层级（crm组织架构）
    List<AgencyTreeDTO> getProjectAgencyListAll();
    List<AgencyTreeDTO> getProjectAgencyListAllByIds(List<String> updateProject);
    ...
    //获取组织架构关系
    List<AgencyTreeDTO> getProjectOAAgencyMessage();

    //获取组织架构关系只显示甲方
    List<AgencyTreeDTO> getOAAgencyMessageProject();

    //删除授权关系
    void deleteProjectAccredit(String id);

    //通过Id获取机构下的人
    List<AgencyTreeDTO> getUserByAgencyId(String agencyId, String category);

    //通过Id获取项项目层级下的角色
    List<AgencyTreeDTO> getProjectRoleByAgencyId(String agencyId);
