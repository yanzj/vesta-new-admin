package com.maxrocky.vesta.application.projectAuthority.impl;
 ...
 */
@Service
public class ProjectUserAuthorityServiceImpl implements ProjectUserAuthorityService {

    @Autowired
    private AgencyRepository agencyRepository;
    @Autowired
    private ProjectUserAuthorityRepository projectUserAuthorityRepository;
    @Autowired
    ...
    public List<UserStaffDTO> getStaffListByAgencyId(String agencyId) {
        ...
        if(null != agencyId) {
            ...
            List<UserMapEntity> userMapEntities = projectUserAuthorityRepository.getUserMap();
    ...
    @Override
    public List<UserStaffDTO> conditionQueryProjectUser(UserStaffDTO userStaffDTO, WebPage webPage) {
        Map map = new HashMap();
        ...
        List<Object[]> list = projectUserAuthorityRepository.getOAUserManageListByCondition(map,webPage,idList);
        for(Object[] obj : list){
    ...
    @Override
    public void startInsideProjectStaff(StaffBatchDTO staffBatchDTO) {
        if(!StringUtil.isEmpty(staffBatchDTO.getStaffIds())){
            ...
            for(String staffId : list){
                ...
                projectUserAuthorityRepository.saveOrUpdateProjectStaff(userMapEntity);
            }
        }
    }

    @Override
    public List<AgencyTreeDTO> getProjectOuterAgencyList() {
        List<AgencyTreeDTO> agencyTreeDTOs = new ArrayList<AgencyTreeDTO>();
        List<AuthOuterAgencyEntity> authOuterAgencyEntities = projectUserAuthorityRepository.getProjectOuterAgencyList();
    ...
    @Override
    public List<AgencyTreeDTO> getProjectOuterAgencyListById(String staffIdW) {
        List<AgencyTreeDTO> agencyTreeDTOs = new ArrayList<AgencyTreeDTO>();
        List<AuthOuterAgencyEntity> authOuterAgencyEntities = projectUserAuthorityRepository.getProjectOuterAgencyList();
        List<String> agencyIds = projectUserAuthorityRepository.getProjectAgencyByStaffId(staffIdW);
        if(null != authOuterAgencyEntities && authOuterAgencyEntities.size()>0){
            AgencyTreeDTO agencyTreeDTO;
            for(AuthOuterAgencyEntity authOuterAgencyEntity : authOuterAgencyEntities){
                agencyTreeDTO = new AgencyTreeDTO();
    ...
    @Override
    public List<OuterUserDTO> getProjectOuterUserList(OuterUserDTO outerUserDTO, WebPage webPage) {
        ...
        List<Object[]> list = projectUserAuthorityRepository.getProjectOuterUserList(map,webPage);
        for(Object[] obj : list){
            ...
            List<String> agencyNameList = projectUserAuthorityRepository.getProjectAgencyNameByStaffId(outerUserDTO1.getStaffIdO());
    ...
    @Override
    public void toDeleteProjectUser(String staffId) {
        ...
        UserMapEntity userMapEntity = projectUserAuthorityRepository.getUserMapById(staffId);
    ...
    @Override
    public void batchDeleteProjectUser(StaffBatchDTO staffBatchDTO) {
        if(!StringUtil.isEmpty(staffBatchDTO.getStaffIds())){
            String str[] = staffBatchDTO.getStaffIds().split(",");
            List<String> list = Arrays.asList(str);
            for(String staffId : list){
                toDeleteProjectUser(staffId);
            }
        }
    }

    @Override
    public List<EnabledUserDTO> getProjectEnabledUserList(EnabledUserDTO enabledUserDTO, WebPage webPage) {
        Map map = new HashMap();
        map.put("agencyName",""); //所属机构
        map.put("staffName","");//人员
        ...
        List<Object[]> list = projectUserAuthorityRepository.getProjectEnabledUserList(map,webPage);
        for(Object[] obj : list){
    ...
    @Override
    public Map getProjectAgencys() {
        List<AuthOuterAgencyEntity> list = projectUserAuthorityRepository.getProjectOuterAgencyList();
    ...
    @Override
    public UserManageDTO getProjectUserManage(String staffId) {
        List<UserAgencyEntity> userAgencyEntities = agencyRepository.getUserAgency(staffId);
        UserMapEntity userMapEntity = projectUserAuthorityRepository.getUserMapById(staffId);
        List<UserInformationEntity> userInformationEntities = agencyRepository.getUserInformation(staffId);
    ...
    @Override
    public String updateProjectStaff(UserManageDTO userManageDTO, UserInformationEntity userInformation) {
        ...
        if(null != userManageDTO.getStatusW()){
            UserMapEntity userMapEntity = projectUserAuthorityRepository.getUserMapById(userManageDTO.getStaffIdW());
    ...
    @Override
    public String saveProjectStaff(UserManageDTO userManageDTO, UserInformationEntity userInformation) {
    ...
    @Override
    public ApiResult saveProjectOuterAgency(OuterAgencyDTO outerAgencyDTO, UserInformationEntity userPropertyStaffEntity) {
        if(null != outerAgencyDTO){
            ...
            if(null != outerAgencyDTO.getAgencyId()){
                ...
                String str[] = agencyPath.split(",");
                List<String> list = Arrays.asList(str);
                if(list.contains("12fc5b60e19f471cad90068e7fcc594a")){//工程临时组id
                    authOuterAgencyEntity.setIsTemporary("1");//是临时组
                    authOuterAgencyEntity.setTemporaryScope("es");//临时组生效范围
                }
    ...
    @Override
    public ApiResult delProjectOuterAgency(String agencyId, UserInformationEntity userPropertyStaffEntity) {
        try{
            if(null != agencyId){
                if("12fc5b60e19f471cad90068e7fcc594a".equals(agencyId) || "03d3df6a599747ef9bfa4332c0f919b6".equals(agencyId)){
    ...
    @Override
    public void projectEnabledUserExcel(String title, String[] headers, ServletOutputStream out, EnabledUserDTO enabledUserDTO) {
        WebPage webPage = new WebPage();
        webPage.setPageIndex(0);
        webPage.setPageSize(1000);

        List<EnabledUserDTO> enabledUserDTOS = this.getProjectEnabledUserList(enabledUserDTO,webPage);
    ...
    @Override
    public void projectOuterUserExcel(String title, String[] headers, ServletOutputStream out, OuterUserDTO outerUserDTO) {
        WebPage webPage = new WebPage();
        webPage.setPageIndex(0);
        webPage.setPageSize(1000);

        List<OuterUserDTO> outerUserDTOList = this.getProjectOuterUserList(outerUserDTO,webPage);
    ...
    @Override
    public String importProjectOuterPeopleExcel(InputStream fis, UserInformationEntity userPropertyStaffEntity) {
        try{
            //创建Excel工作薄
            ...
            for (int i = 0; i < hwb.getNumberOfSheets(); i++) {
                ...
                for (int j = 1; j < sheet.getPhysicalNumberOfRows(); j++) {
                    ...
                    if (row.getCell(0) != null) {//第一列数据 组织架构
                        ...
                        //查看组织架构名是否已存在
                        AuthOuterAgencyEntity authOuterAgencyEntity1 = projectUserAuthorityRepository.getOuterAgencyByName(agencyName);
                        if (authOuterAgencyEntity1 != null) {//如果有则使用已有ID
                            if (row.getCell(3) != null) {//第四列数据 联系方式 做为用户名
                                ...
                                UserInformationEntity userInformationEntity = projectUserAuthorityRepository.getStaffBySysName(userNameR);//根据用户名查询是否已被注册
                                if (userInformationEntity != null) {
                                    ...
                                    boolean savestaff = projectUserAuthorityRepository.addProjectOuterStaff(userInformationEntity1);//保存人员信息
                                    if (savestaff) {
                                        UserAgencyEntity userAgencyEntity = new UserAgencyEntity();
                            ...
                            if (row.getCell(3) != null) {//第4列数据 用户名
                                String userNameR = getCellValue(row.getCell(3)).trim();
                                UserInformationEntity userInformationEntity = projectUserAuthorityRepository.getStaffBySysName(userNameR);//根据用户名查询是否已被注册s
                                if (userInformationEntity != null) {
                                    return "错误！第" + (j + 1) + "行第4列用户名已被注册！";
                                }else {
                                    UserInformationEntity userInformationEntity1= new UserInformationEntity();
